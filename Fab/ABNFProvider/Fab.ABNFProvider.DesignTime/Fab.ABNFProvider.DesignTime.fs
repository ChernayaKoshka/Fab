...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Fab.ABNFProvider.DesignTime", "Fab.ABNFProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Fab.ABNFProvider.DesignTime", "Fab.ABNFProvider.Runtime")])

    let ns = "Fab.ABNFProvider"
    let asm = Assembly.GetExecutingAssembly()
